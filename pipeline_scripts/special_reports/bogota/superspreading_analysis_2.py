...
df_dist_infected = bqf.get_distance_to_infected(
    client, location_graph_id, min_date, max_date)
df_dist_infected.date = pd.to_datetime(df_dist_infected.date)
dist_infected_dates = df_dist_infected.date.unique()


print(ident +
      f'   Computing Centralities from: {min_date.strftime(date_format)} to {max_date.strftime(date_format)}')
...
while current_date <= max_date:
    ...
    date_closest_dist_infected = dist_infected_dates[np.argmin(
        [np.abs((current_date - d).days) for d in dist_infected_dates])]

    # Extracts the distaace to infected temporary
    df_dist_infected_temp = df_dist_infected[df_dist_infected.date ==
                                             date_closest_dist_infected]
    ...
    for hour in range(6, 22):
        ...
        # Declares the edges
        nodes_from_edges = pd.concat((edges[['id1']].rename(columns={'id1': 'identifier'}), edges[[
                                     'id2']].rename(columns={'id2': 'identifier'})), ignore_index=True).drop_duplicates()
        ...
        nodes['weight'] = np.log(
            1 + np.exp(-1*nodes.distance_to_infected/div))/np.log(2)

        # Edges
        # Groups
        compact_edges = edges[['id1', 'id2']].copy()
        compact_edges['weight'] = 1
        compact_edges = compact_edges.groupby(
            ['id1', 'id2']).count().reset_index()

        # Gets the personalized pagerank
        nodes['centrality'] = grf.get_personalized_ppr(
            nodes, compact_edges, weighted_edges=True)
...
geo_localidades = gpd.read_file(os.path.join(
    data_dir, 'data_stages', location_folder_name, 'raw/geo/', 'localities_shapefile.shp'),)
geo_localidades.geometry = geo_localidades.geometry.set_crs("EPSG:4326")

# Constructs boundingbox
centroids = geo_localidades.geometry.to_crs(
    'EPSG:3395').centroid.to_crs("EPSG:4326")
...
df1 = df_locations[['lat', 'lon', 'total_contacts']].rename(
    columns={'total_contacts': 'total'})
...
geo_locations = gpd.GeoDataFrame(
    df1, crs="EPSG:4326", geometry=gpd.points_from_xy(df1.lon, df1.lat))
...
df2 = df2.groupby(['lat', 'lon']).sum().reset_index().sort_values(
    'total', ascending=False).head(total_places)
...
geo_pagerank_trace = gpd.GeoDataFrame(
    df3, crs="EPSG:4326", geometry=gpd.points_from_xy(df3.lon, df3.lat))
...
print(ident + "   Plots")
# Localities
ax = geo_localidades.rotate(rotate, origin=centr).plot(
    figsize=(6, 10), color="black", alpha=0.6, zorder=1)
geo_localidades.rotate(rotate, origin=centr).boundary.plot(
    ax=ax, color="white", alpha=0.4, zorder=1)
...
df = pd.DataFrame({'geometry': [geo_top_both.rotate(
    rotate, origin=centr).buffer(buffer_lugares_superdispersion).unary_union]})
...
ax.figure.savefig(os.path.join(export_folder_location,
                  'superdispersion_bogota.png'), dpi=150)
