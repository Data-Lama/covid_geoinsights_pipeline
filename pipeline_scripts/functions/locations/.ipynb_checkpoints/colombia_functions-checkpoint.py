...
class Unifier(GenericUnifier):
	...
	def build_cases_geo(self):
		...
		df = pd.read_csv(file_name, parse_dates = ['Fecha diagnostico'], date_parser = lambda x: pd.to_datetime(x, errors="coerce"))
	...
	def __update_geo(self, max_tries = 3):
		'''
		...
		cases['location'] = cases.apply(lambda row: '{}-{}'.format(row.city, row.state), axis = 1)

		
		# Finds missing locations
		merged = cases.merge(df_geo, on = ['geo_id'], how = 'left')
		muni_ids = merged.loc[merged.lon.isna()].geo_id.unique()

		
		if len(muni_ids) == 0:
			print('Found zero new places.')
		else:
			print('Found {} new places. Updating'.format(len(muni_ids)))
			print('')

			# Loads municipios dataset
			location = os.path.join(self.raw_folder, 'geo', self.get('geo_file_name'))
			df_muni = pd.read_csv(location)
			df_muni.index = df_muni.muni_id.apply(str)
		
			for geo_id in muni_ids:

				if geo_id not in df_muni.index:
					print('   Muni ID not found: {}. Places: '.format(geo_id))

					df_temp = cases.loc[cases.geo_id == geo_id].location.value_counts().to_frame()
					
					for ind, row in df_temp.iterrows():
						print('      ' + '{}: {}'.format(ind, row.location))
					
					print()
					
					continue

				row = df_muni.loc[geo_id]

				print('   Finding: {}, {}'.format(row.muni_name, row.dep_name))
				tries = 0
				while tries < max_tries:
					try:
						lon, lat = geo.geolocate(country, row.dep_name, row.muni_name)
						print('      Done! Location: {},{}'.format(lat,lon))
						geo.save_new_location(row.geo_id, country, row.dep_name, row.muni_name, lon, lat)
						break
