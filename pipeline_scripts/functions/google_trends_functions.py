...
from gloogletrans import Translator
...
def translate_terms(list_terms_EN, dest_len):
    res_list = []
    # Check if keywork 'covid' is in list
    if 'covid' in list_terms_EN:
        list_terms_EN.remove('covid')
        res_list.append('covid')
    # Translate
    translator = Translator()
    result = translator.translate(list_terms_EN, src='en', dest=dest_len)
    translated_res = [trans.text for trans in result]
    res_list.extend(translated_res)
