...
class GenericUnifier():
	...
	def __init__(self, location_name, location_folder_name):
		...
		# Creates the folders
		# Raw
		self.raw_folder = os.path.join(global_dir, 'data/data_stages/', location_folder_name, 'raw')
		# Creates the folder if does not exists
		if not os.path.exists(self.raw_folder):
			os.makedirs(self.raw_folder)

		# Unified
		self.unified_folder = os.path.join(global_dir, 'data/data_stages/', location_folder_name, 'unified')
		# Creates the folder if does not exists
		...
		# Unified
		self.agglomerated_folder = os.path.join(global_dir, 'data/data_stages/', location_folder_name, 'agglomerated')
		# Creates the folder if does not exists
		if not os.path.exists(self.agglomerated_folder):
			os.makedirs(self.agglomerated_folder)

		# Constructed
		self.constructed_folder = os.path.join(global_dir, 'data/data_stages/', location_folder_name, 'constructed')
		# Creates the folder if does not exists
		...
		# Description
		self.df_description = pd.read_csv(os.path.join(global_dir, 'data/data_stages/', location_folder_name, 'description.csv'), index_col = 0)
