...
# Sets the location
parent_dir = Path(os.path.realpath(__file__)).parent.parent.parent

temp_folder = os.path.join(parent_dir, 'data/downloads/facebook_movement_{}/'.format(datetime.now().strftime('%Y_%m_%d_%H_%M_%S')))
#temp_folder = os.path.join(parent_dir, 'pipeline_scripts/PRE/temp_folder_{}/'.format(12826453))
configuration_folder = os.path.join(parent_dir, 'pipeline_scripts/configuration/')
...
if selected_locations.shape[0] == 0:
	...
	for ind, row in selected_locations.iterrows():
		...
		if len(m) > 0:
			...
			# Folder
			destination_folder =  os.path.join(parent_dir, 'data/data_stages/',row.location_folder, 'raw/', fb.get_folder_name_by_type(row.type_data))
			# Makes Directory
