...
for(agglomeration_method in agglomeration_methods)
{
	...
	# Export Folder
	export_folder = file.path(export_folder, 'agglomerated_polygons_plots')
	dir.create(export_folder)
	...
	# Agglomerated
	df_movement_agg = read.csv(file.path(agglomerated_folder,'movement.csv'), stringsAsFactors = FALSE)
	df_poly = read.csv(file.path(agglomerated_folder,'polygons.csv'), stringsAsFactors = FALSE)
	...
	# Saves
	ggsave(file.path( export_folder, paste0("selected_polygons_", agglomeration_method,".jpeg")), plot = p, width = width, height = height, device = 'jpeg')
	...
	export_folder_movement_selected = file.path(export_folder, paste0('selected_polygons_movement_by_month_', agglomeration_method))
	...
	for(m in sort(unique(df_movement_agg$month)))
	{
		...
		for(w in sort(unique(df_mes$week)))
		{
		  ...
		  # Saves
		  ggsave(file.path( export_folder_movement_selected, paste0("movement_selected_", agglomeration_method, "_", m,'_', w,".jpeg")), plot = p, width = width_selected, height = height_selected, device = 'jpeg')
	...
	# Saves
	ggsave(file.path( export_folder, paste0("movement_selected_", agglomeration_method, "_last_days.jpeg")), plot = p, width = width_selected, height = height_selected, device = 'jpeg')
