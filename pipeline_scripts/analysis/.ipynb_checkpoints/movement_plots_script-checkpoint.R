...
for(agglomeration_method in agglomeration_methods)
{
  ...
  # Export Folder
  export_folder = paste(export_folder, 'movement_plots/', sep = "")
  dir.create(export_folder)
  
  
  # Unified Folder
  unified_folder = paste('data/data_stages/',location_folder,'/unified/', sep = "")
  # Agglomerated Folder
  agglomerated_folder = paste('data/data_stages/',location_folder,'/agglomerated/', agglomeration_method, '/', sep = "")
  ...
  # Unified
  df_movement_unified = read.csv(paste(unified_folder,'movement.csv', sep = ''), stringsAsFactors = FALSE)
  ...
  bottom = min(quantile(df_movement_unified$start_movement_lat, alfa), quantile(df_movement_unified$end_movement_lat, alfa)) 
  top = max(quantile(df_movement_unified$start_movement_lat, 1- alfa), quantile(df_movement_unified$end_movement_lat, 1-  alfa)) 
  left = min(quantile(df_movement_unified$start_movement_lon, alfa), quantile(df_movement_unified$end_movement_lon), alfa)
  right = max(quantile(df_movement_unified$start_movement_lon, 1 - alfa), quantile(df_movement_unified$end_movement_lon, 1- alfa)) 
  ...
  export_folder_movement_change = paste(export_folder,'/movement_change_by_month/', sep = "")
  ...
  for(m in sort(unique(df_movement_unified$month)))
  {
    ...
    for(w in sort(unique(df_mes$week)))
    {
      ...
      # Saves
      ggsave(paste( export_folder_movement_change, "movement_change_",m,'_', w,".jpeg", sep = ""), plot = p, width = width_change, height = height_change, device = 'jpeg')
  ...
  # Saves
  ggsave(paste( export_folder, "movement_change_last_days.jpeg", sep = ""), plot = p, width = width_change, height = height_change, device = 'jpeg')
